...
class MetaPolicyExperimentEnv(VecTask):
    ...
    def reset_idx(self, env_ids):
        ...
        num_props =10  # 1:10
        # 거리: 1 ~ 5m
        radii = 1 + torch.rand(num_props) * 3
        # 방향: 0 ~ 2π (rad)
        angles = torch.rand(num_props) * 2 * np.pi

        # 위치 계산 (x, y)
        x_offsets = radii * torch.cos(angles)
        y_offsets = radii * torch.sin(angles)
        positions = torch.stack([x_offsets, y_offsets], dim=-1)  # shape: (9, 2)

        # 회전 각도 (0 ~ 2π)
        orientations = torch.rand(num_props) * 2 * np.pi  # shape: (9,)

        # 위치 설정
        #random_prop_init_pos[0, :, 0:2] = positions  # (x, y)
        ...
        random_prop_init_pos[0, 5, 0:2] = torch.tensor([-0.9, 2.3], device=self.device)
        random_prop_init_pos[0, 6, 0:2] = torch.tensor([2.7, -2.7], device=self.device)
        random_prop_init_pos[0, 7, 0:2] = torch.tensor([0.7, 0.5], device=self.device)
        random_prop_init_pos[0, 8, 0:2] = torch.tensor([3.65, -0.5], device=self.device)
        random_prop_init_pos[0, 9, 0:2] = torch.tensor([-2.5, 1.5], device=self.device)

        # orientation 설정 (예: z축 회전만 있는 3D quaternion으로 변환)
        # 회전축은 항상 z축: (0, 0, sin(θ/2), cos(θ/2))
        ...
        #    random_prop_init_pos[i, 1:10, 0:2] = (torch.rand(self.num_boxes-1, 2) - 0.5) * torch.tensor([1*self.env_space, 1*self.env_space])
        ...
        self.root_states[self.prop_indices[env_ids[0]*self.num_boxes:env_ids[0]*self.num_boxes+self.num_boxes]] = random_prop_init_pos[0]
