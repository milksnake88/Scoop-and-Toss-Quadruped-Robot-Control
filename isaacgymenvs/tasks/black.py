...
class Black(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.projected_gravity = torch.zeros_like(self.gravity_vec, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.reset_progress_s[:] = 7

        self.commands = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
        self.commands[:, 0] = 0.2
        # x, y 속도의 합과 카운트를 저장할 변수 추가
        self.base_vel_sum = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.base_vel_count = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_dof):
            ...
            a1_dof_props['stiffness'][i] = 60 #60.
            a1_dof_props['damping'][i] = 3 #3.
    ...
    def post_physics_step(self):
        ...
        rand_prob = torch.rand(self.num_envs, device=self.device)
        envs_to_change = torch.where(self.episode_cnt.squeeze() > 4/self.dt)[0]
        if len(envs_to_change) > 0:
            self.set_random_goal_velocity(envs_to_change)
        self.base_vel_sum += self.a1_root_states[:, 7:9]
        self.base_vel_count += 1
    ...
    def compute_observations(self):
        ...
        command = quat_rotate_inverse(base_quat, self.commands)
    ...
    def set_random_goal_velocity(self, env_ids):
        num_envs = len(env_ids)
        speeds = np.random.uniform(0.3, 1.0, size=(num_envs, 1))  # 0.3 ~ 1.0 랜덤 속도
        angles = np.random.uniform(-np.pi, np.pi, size=(num_envs, 1))  # -π ~ π 랜덤 방향

        velocities = np.hstack((speeds * np.cos(angles), speeds * np.sin(angles)))  # 속도 벡터 계산
        velocities = torch.tensor(velocities, dtype=torch.float, device=self.device)

        self.commands[env_ids, :2] = velocities  # 목표 속도 업데이트
    ...
    def update_goal_position(self):
        """
        목표 속도를 기반으로 목표 위치를 설정하고, 해당 위치를 가상의 물체(타겟)로 나타냅니다.
        """
        time_horizon = 4  # 로봇이 목표 속도를 유지하면 1.5초 후 도달할 위치를 목표 위치로 설정
        scale = time_horizon * 1.0  # 1.5초 후 목표 속도를 고려하여 목표 위치 계산

        # 현재 로봇 베이스 위치 (shape: (num_envs, 3))
        base_positions = self.a1_root_states[:, 0:3]

        # 목표 속도 (로봇의 좌표계 기준 → 글로벌 좌표로 변환)
        goal_velocity_global = self.commands

        # 목표 위치 계산 (현재 위치 + 속도 * 시간)
        goal_positions = base_positions + scale * goal_velocity_global

        # 목표 위치를 물체(타겟)의 위치로 설정
        self.root_states[self.prop_indices[:], 0:3] = goal_positions

        actor_indices = self.all_actor_indices[:].flatten()
        self.gym.set_actor_root_state_tensor_indexed(self.sim,
                                                     gymtorch.unwrap_tensor(self.root_states),
                                                     gymtorch.unwrap_tensor(actor_indices), len(actor_indices))

        # 시뮬레이터에 업데이트된 목표 위치 반영
    ...
    def reset_idx(self, env_ids):
        ...
        self.set_random_goal_velocity(env_ids)
