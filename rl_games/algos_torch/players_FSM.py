...
class PpoPlayerFSM(BasePlayer):
    ...
    def get_expert_action(self, expert, obs, shovel_to_prop_dis, is_deterministic=False):
        ...
        proprioception = processed_obs[:, 9:] #TODO: cfg로 받기(map size 달라질수도 있음)
        ...
        obs_for_experts = torch.cat((closest_prop_pos, shovel_to_prop_dis, projected_gravity, proprioception), dim=-1)
        expert.eval()
        input_dict = {
            'is_train': False,
            'prev_actions': None,
    ...
    def run(self):
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                ...
                """
                selected_action = torch.where(self.base_to_prop_dis < 10, e2_action, e1_action)


                if (self.base_to_prop_dis < 0.6) or (self.prop_in_air):
                    selected_action = e2_action
                    print("action 2")
                else:
                    selected_action = e1_action
                    print("action 1")
                """
                ...
                obses, r, done, info = self.env_step(self.env, selected_action)
                cr += r
                steps += 1
