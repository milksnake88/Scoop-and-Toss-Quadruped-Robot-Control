...
class A1WithShovelPassiveJointWithCamera(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cam_props_width = 110
        self.cam_props_height = 72

        self.cfg["env"]["numObservations"] = self.cam_props_width * self.cam_props_height + 48
        self.cfg["env"]["numActions"] = 12

        # box init state. TODO: add to cfg file
        box_pos = [0.44, 0.13, 0.025]
        box_rot = [0., 0., 0., 1.]
        box_v_lin = [0., 0., 0.]
        box_v_ang = [0., 0., 0.]
        ...
        if self.viewer:
            ...
            if self.enable_viewer_sync:
                ...
                if self.render_fps < 0:
                    ...
                    render_dt = self.dt * self.control_freq_inv * 20 # render every control step
                else:
    ...
    def compute_observations(self):
        ...
        self.obs_buf[:] = torch.cat((self.depth_images.to(self.device),
                                     rot_matrix.to(self.device),
                                     base_ang_vel.to(self.device),
                                     accelerometer.to(self.device),
                                     self.dof_pos_new.to(self.device),
                                     self.dof_vel_new.to(self.device),
