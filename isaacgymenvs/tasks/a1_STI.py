...
class A1STI(VecTask):
        ...
        if self.viewer:
            ...
            if self.enable_viewer_sync:
                ...
                if self.render_fps < 0:
                    ...
                    render_dt = self.dt * self.control_freq_inv * 3 # render every control step
                else:
    ...
    def save_STI_info_as_numpy(self, STI_info):
        ...
        if self.step_cnt[0] == self.sample_size+1:
          ...
          np.save('obs_buf/obs_buf_picking_throwing.npy', samples_array)
    ...
    def reset_idx(self, env_ids):
        ...
        obs_buf = self.load_obs_buf_as_tensor().squeeze() # (10000, 52)
        sampled_ids = torch.randint(0, self.sample_size, (len(env_ids), 1), device=self.device)

        self.dof_pos[env_ids] = obs_buf[sampled_ids, 13:26].squeeze()
        self.dof_vel[env_ids] = obs_buf[sampled_ids, 26:39].squeeze()

        # self.root_states[self.a1_indices[env_ids]] = self.a1_init_state[env_ids].clone()
        random_a1_init_state = obs_buf[sampled_ids, 0:13].squeeze()
        self.root_states[self.a1_indices[env_ids]] = random_a1_init_state

        random_prop_init_pos = obs_buf[sampled_ids, 39:52].squeeze()
