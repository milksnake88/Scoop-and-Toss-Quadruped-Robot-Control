...
class A1WithShovel(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #self.stacked_tensor = torch.empty(self.num_envs, 128, 128, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        if self.viewer:
            ...
            if self.enable_viewer_sync:
                ...
                if self.render_fps < 0:
                    ...
                    render_dt = self.dt * self.control_freq_inv * 10 # render every control step
                else:
    ...
    def post_physics_step(self):
        ...
        self.stacked_tensor = torch.stack(self.cam_tensors_flattened, dim=0)
        #print(self.stacked_tensor[0])


        self.gym.start_access_image_tensors(self.sim)
        self.stacked_tensor = torch.stack(self.cam_tensors_flattened, dim=0).view(self.num_envs, 128, 128)
        #print(self.stacked_tensor.shape)
        fname = os.path.join("a1_with_shovel_images", "test6_cam_depth2-%04d.png" % (self.progress_buf[0]))
        #print(self.stacked_tensor)
        depth_img = self.stacked_tensor[0].cpu().numpy()
        print(depth_img)
