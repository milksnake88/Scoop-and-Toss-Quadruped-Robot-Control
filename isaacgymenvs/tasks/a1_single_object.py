...
class A1SingleObject(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        #self.offset_forward[:] = 1.3
        #self.offset_backward[:] = -0.65
        #self.offset_right[:] = 1.04
        #self.offset_left[:] = -1.04
        ...
        self.cnt = 0.
        self.goal_cnt = 0.
        self.goal_ave_time = 0.
        self.num_landing = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.landing_now = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.num_boxes = 1
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #box_asset = self.gym.create_capsule(self.sim, 0.02, 0.015, box_asset_options)
    ...
    def track_packing_times(self):
        ...
        for env_idx in range(self.num_envs):
            if newly_landed[env_idx]: # 새롭게 담긴 경우
    ...
    def reset_idx(self, env_ids):
        ...
        positions_offset = torch_rand_float(0.7, 1.3, (len(env_ids), self.num_dof), device=self.device)
        velocities = torch_rand_float(-0.3, 0.3, (len(env_ids), self.num_dof), device=self.device)
        ...
        prop_position_offset_x = torch_rand_float(-4.0, 4.0, (len(env_ids), 1), device=self.device)
        prop_position_offset_y = torch_rand_float(-4.0, 4.0, (len(env_ids), 1), device=self.device)
        #prop_position_offset_x = torch_rand_float(-0.2, 0.3, (len(env_ids), 1), device=self.device)
        #prop_position_offset_y = torch_rand_float(-0.25, 0.25, (len(env_ids), 1), device=self.device)
        r = 1 + (5-1) * torch.rand(1, device=self.device)
        ...
        #1 담은 개수
        self.metric1 += self.num_landing.float()

        #2 평균 담기 시간
        #self.metric2 += self.avg_packing_time
        if self.num_landing[env_ids]:
          self.goal_cnt += 1
          self.metric2 += self.time
          self.goal_ave_time = self.metric2 / self.goal_cnt

        #3 누락 물체 개수
        self.metric3 += (self.num_landing.float() - temp)

        print("episod: ", self.cnt)
        print(self.metric1)
        #print(self.metric3)
        print(self.goal_ave_time)
