...
class PpoPlayerContinuousHelper(BasePlayer):
    ...
    def load_controller_model(self, params):
        builder = model_builder.ModelBuilder()
        network = builder.load(params)
        # TODO: expert cfg에 추가
        actions_num = 2
    ...
    def get_action(self, obs, is_deterministic = False):
        ...
        if self.clip_actions:
            return rescale_actions(self.actions_low, self.actions_high, torch.clamp(current_action, -1.0, 1.0)), weights
        else:
            return current_action, weights
    ...
    def get_expert_action(self, expert, obs, shovel_to_prop_dis, is_deterministic=False):
        ...
        if self.clip_actions:
            ...
            return current_action, res_dict['values']

    def get_controller_action(self, controller, obs, is_deterministic = False):
        obs = self._preproc_obs(obs)
        self.model.eval()
        input_dict = {
            'is_train': False,
        ...
        with torch.no_grad():
            res_dict = controller(input_dict)
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                ...
                if has_masks:
                    ...
                    action, weight = self.get_action(obses, is_deterministic)

                e1_action, e1_values = self.get_expert_action(self.e1_model, obses, self.shovel_to_prop_dis)
                e2_action, e2_values = self.get_expert_action(self.e2_model, obses, self.shovel_to_prop_dis)
                controller_action = self.get_controller_action(self.controller_model, obses)

                # discrete
                expert_action = controller_action * e1_action + (1-controller_action)*e2_action
                alpha = 0.1
                scaled_action = expert_action + alpha * torch.tanh(action)

                obses, r, done, info = self.env_step(self.env, scaled_action)
                cr += r
                steps += 1

                self.shovel_to_prop_dis = info["shovel_to_prop_dis"]
                self.base_to_prop_dis = info["base_to_prop_dis"]
                self.projected_gravity = info["projected_gravity"]
                self.prop_in_air = info["prop_in_air"]
