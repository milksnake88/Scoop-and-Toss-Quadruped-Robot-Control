...
class A1Throwing(A1WithShovelThrowing):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.prop_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.prop_handles[0], "prop") #for A1WithShovelDaggerPassiveJoint
    ...
    def compute_reward(self):
        ...
        # distance reward
        bed_position = self.get_transformed_position(point_local=self.bed_position_local) #for A1WithShovelDaggerPassiveJoint
        prop_position = self.prop_root_states[:, 0:3] #for A1WithShovelDaggerPassiveJoint
        bed_prop_distance = torch.norm(prop_position - bed_position, dim=1)
        rew_bed_prop_distance = torch.exp(-bed_prop_distance/0.25)

        # throwing up reward
        box_position_upward = self.prop_root_states[:, 2] #for A1WithShovelDaggerPassiveJoint
        ...
        total_reward = rew_landing + (30 * rew_bed_prop_distance * rew_box_position_upward)\
                       -0.001 * rew_action_rate -0.0001 * rew_joint_acc -0.00003 * rew_torque #- 0.001 * rew_hip_pos
        #total_reward = 15 * rew_prop_is_picked + rew_landing + (30 * rew_bed_prop_distance * rew_box_position_upward)\
        ...
        margin = 0.1
        joint_violation = (self.dof_pos < (self.lower_dof_limits + margin)) | (self.dof_pos > (self.upper_dof_limits - margin))
        termination_mask = joint_violation.any(dim=1)
        # 기본 reset 조건 (벡터 연산으로 모두 계산)
