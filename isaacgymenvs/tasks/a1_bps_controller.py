...
class A1BPSController(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        #self.cfg["env"]["numObservations"] = 80*3 + 49
        ...
        self.landing = torch.zeros(self.num_envs, self.num_props, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.cube_mesh = load_objs_as_meshes(["/home/mj/IsaacGym/IsaacGymEnvs/assets/urdf/objects/meshes/cube_multicolor.obj"], device=self.device)  # CUDA 장치에 바로 로드
        self.banana_mesh = load_objs_as_meshes(["/home/mj/IsaacGym/IsaacGymEnvs/assets/urdf/ycb/011_banana/collision.obj"], device=self.device)
        self.mug_mesh = load_objs_as_meshes(["/home/mj/IsaacGym/IsaacGymEnvs/assets/urdf/objects/meshes/bucket.obj"], device=self.device)
        self.brick_mesh = load_objs_as_meshes(["/home/mj/IsaacGym/IsaacGymEnvs/assets/urdf/ycb/061_foam_brick/textured.obj"], device=self.device)

        self.cube_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.prop_handles[0], "object") #27
        self.bucket_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.prop_handles[1], "bucket")
        ...
        self.can_index = self.gym.find_actor_rigid_body_handle(self.envs[0], self.prop_handles[5], "can")
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            for j in range(1):
                cube_handle = self.gym.create_actor(env_ptr, object_assets[0], object_pose, "cube"+str(j), i, 0, 0)
                bucket_handle = self.gym.create_actor(env_ptr, object_assets[1], object_pose, "bucket"+str(j), i, 0, 0)
                banana_handle = self.gym.create_actor(env_ptr, object_assets[2], object_pose, "banana"+str(j), i, 0, 0)
                mug_handle = self.gym.create_actor(env_ptr, object_assets[3], object_pose, "mug"+str(j), i, 0, 0)
                brick_handle = self.gym.create_actor(env_ptr, object_assets[4], object_pose, "brick"+str(j), i, 0, 0)
                can_handle = self.gym.create_actor(env_ptr, object_assets[5], object_pose, "can"+str(j), i, 0, 0)
    ...
    def compute_observations(self):
        ...
        pos_of_prop_wrt_a1_base = self.get_transformed_position(self.a1_root_states, point_global=closest_prop_pos)
        bps_enc = self.get_bps(self.prop_root_states[torch.arange(self.num_envs), min_distance_indices.squeeze()]).reshape(self.num_envs, -1)
        ...
        self.obs_buf[:] = torch.cat((#bps_enc,
    ...
    def reset_idx(self, env_ids):
        ...
        self.prop_num[env_ids] = np.random.randint(0, 4)
        ...
        for i in range(env_ids.size(0)):
            random_prop_init_pos[i, :, 0:2] = (torch.rand(self.num_props, 2) - 0.5) * torch.tensor([self.env_space-0.5, self.env_space-0.5])
