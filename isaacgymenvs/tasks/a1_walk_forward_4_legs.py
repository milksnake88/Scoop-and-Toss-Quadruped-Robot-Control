...
class A1WalkForward4Legs(A1):
  ...
  def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
    ...
    line_v = [[self.env_origin.x, self.env_origin.y, self.env_origin.z],
              [100., 0., 0.]]
  ...
  def compute_base_angle_reward(self):
    ...
    if (self.num_envs > 1):
      for i in range(self.num_envs):
        trunk_vector = gymapi.Vec3(midle_of_F_hips[i, 0] - midle_of_R_hips[i, 0],
      ...
      trunk_vector = gymapi.Vec3(midle_of_F_hips[0] - midle_of_R_hips[0],
...
def compute_a1_reward(
    ...
    rew_lin_vel_xy = torch.exp(-lin_vel_error/0.25) * rew_scales["lin_vel_xy"] # [4096]
    ...
    # base angle reward
    rew_base = cos_sim * 0.12

    total_reward = rew_lin_vel_xy + rew_torque + rew_y_pos + rew_base
